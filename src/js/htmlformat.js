  ...
  function Parser() {
    ...
    this.get_tag = function () { //function to get a full tag and parse its type
      ...
      do {
        ...
        //wjw_note: if (char === '=') { //no space before =
        //wjw_add:
        if (char === '=' && content.join().indexOf('<,!,-,-') != 0 ) { //no space before =
          space = false;
        }
        
        //wjw_note: if (content.length && content[content.length-1] !== '=' && char !== '>' && space) { //no space after = or before >
        //wjw_add:
        if (content.length && char !== '>' && space) { //no space after = or before >
    ...
    this.get_unformatted = function (delimiter, orig_tag) { //function to return unformatted content in its entirety
      ...
      do {
        ...
        if (this.pos >= this.input.length) {
        	try {
        	  alert('Bad Html Document:'+orig_tag);
        	  throw ('Bad Html Document:'+orig_tag);
        	} catch(ex) {  
        	  throw ('Bad Html Document:'+orig_tag);
        	}
        }
